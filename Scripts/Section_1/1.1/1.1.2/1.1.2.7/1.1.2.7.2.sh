...
if [[ -n "$mount_output" ]]; then
  if echo "$mount_output" | grep -v 'nodev' >/dev/null 2>&1; then
    reason_for_failure="- The /var/log/audit partition does not have the 'nodev' mount option set. Current mount options: $mount_output"
  else
    audit_result="PASS"
    correct_settings="- The /var/log/audit partition has the 'nodev' mount option set: $mount_output"
  fi
else
