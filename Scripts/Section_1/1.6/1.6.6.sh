...
# Function to check /etc/issue.net permissions
check_issue_permissions() {
    if [ -e "/etc/issue.net" ]; then
        # Get file permissions, UID, and GID
        file_stat=$(stat -Lc '%#a %u %g' /etc/issue.net)
        ...
        if [[ "$file_perm" -le 644 && "$file_uid" -eq 0 && "$file_gid" -eq 0 ]]; then
            a_output+=(" - /etc/issue.net has correct permissions and ownership")
        else
            a_output2+=(" - Incorrect permissions or ownership for /etc/issue.net (Current: $file_stat)")
        fi
    else
        a_output+=(" - /etc/issue.net does not exist")
    fi
}
