...
expected_key="delete"
expected_rules=(
    "-a always,exit -F arch=b64 -S unlink,unlinkat,rename,renameat -F auid>=$UID_MIN -F auid!=unset -k $expected_key"
    "-a always,exit -F arch=b32 -S unlink,unlinkat,rename,renameat -F auid>=$UID_MIN -F auid!=unset -k $expected_key"
)
running_rules=(
    "-a always,exit -F arch=b64 -S rename,unlink,unlinkat,renameat -F auid>=$UID_MIN -F auid!=-1 -F key=$expected_key"
    "-a always,exit -F arch=b32 -S unlink,rename,unlinkat,renameat -F auid>=$UID_MIN -F auid!=-1 -F key=$expected_key"
)
...
[ -n "$UID_MIN" ] && {
    disk_cmd="awk '/^ *-a *always,exit/ &&/ -F *arch=b(32|64)/ &&(/ -F *auid!=unset/||/ -F *auid!=-1/||/ -F *auid!=4294967295/) &&/ -F *auid>=$UID_MIN/ &&/ -S/ &&(/unlink/||/rename/||/unlinkat/||/renameat/) &&(/ key= *[!-~]* *$/||/ -k *[!-~]* *$/) {print}' /etc/audit/rules.d/*.rules"
    running_cmd="auditctl -l | awk '/^ *-a *always,exit/ &&/ -F *arch=b(32|64)/ &&(/ -F *auid!=unset/||/ -F *auid!=-1/||/ -F *auid!=4294967295/) &&/ -F *auid>=$UID_MIN/ &&/ -S/ &&(/unlink/||/rename/||/unlinkat/||/renameat/) &&(/ key= *[!-~]* *$/||/ -k *[!-~]* *$/) {print}'"
}
...
if [ ${#a_output[@]} -eq 0 ] && [ ${#a_output2[@]} -eq 4 ] && [ -n "$UID_MIN" ]; then
