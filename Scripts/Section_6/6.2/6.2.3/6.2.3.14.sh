...
expected_key="MAC-policy"
expected_rules=(
    "-w /etc/apparmor/ -p wa -k $expected_key"
    "-w /etc/apparmor.d/ -p wa -k $expected_key"
)
...
disk_cmd="awk '/^ *-w/ &&(/\/etc\/apparmor/ ||/\/etc\/apparmor.d/) &&/ +-p *wa/ &&(/ key= *[!-~]* *$/||/ -k *[!-~]* *$/) {print}' /etc/audit/rules.d/*.rules"
running_cmd="auditctl -l | awk '/^ *-w/ &&(/\/etc\/apparmor/ ||/\/etc\/apparmor.d/) &&/ +-p *wa/ &&(/ key= *[!-~]* *$/||/ -k *[!-~]* *$/) {print}'"
