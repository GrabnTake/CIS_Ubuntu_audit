...
expected_key="mounts"
expected_rules=(
    "-a always,exit -F arch=b64 -S mount -F auid>=$UID_MIN -F auid!=unset -k $expected_key"
    "-a always,exit -F arch=b32 -S mount -F auid>=$UID_MIN -F auid!=unset -k $expected_key"
)
running_rules=(
    "-a always,exit -F arch=b64 -S mount -F auid>=$UID_MIN -F auid!=-1 -F key=$expected_key"
    "-a always,exit -F arch=b32 -S mount -F auid>=$UID_MIN -F auid!=-1 -F key=$expected_key"
)
...
[ -n "$UID_MIN" ] && {
    disk_cmd="awk '/^ *-a *always,exit/ &&/ -F *arch=b(32|64)/ &&(/ -F *auid!=unset/||/ -F *auid!=-1/||/ -F *auid!=4294967295/) &&/ -F *auid>=$UID_MIN/ &&/ -S/ &&/mount/ &&(/ key= *[!-~]* *$/||/ -k *[!-~]* *$/) {print}' /etc/audit/rules.d/*.rules"
    running_cmd="auditctl -l | awk '/^ *-a *always,exit/ &&/ -F *arch=b(32|64)/ &&(/ -F *auid!=unset/||/ -F *auid!=-1/||/ -F *auid!=4294967295/) &&/ -F *auid>=$UID_MIN/ &&/ -S/ &&/mount/ &&(/ key= *[!-~]* *$/||/ -k *[!-~]* *$/) {print}'"
}
