...
expected_rules=(
    "-w /etc/group -p wa -k $expected_key"
    "-w /etc/passwd -p wa -k $expected_key"
    "-w /etc/gshadow -p wa -k $expected_key"
    "-w /etc/shadow -p wa -k $expected_key"
    "-w /etc/security/opasswd -p wa -k $expected_key"
    "-w /etc/nsswitch.conf -p wa -k $expected_key"
    "-w /etc/pam.conf -p wa -k $expected_key"
    "-w /etc/pam.d -p wa -k $expected_key"
)
...
disk_cmd="awk '/^ *-w/ &&(/\/etc\/group/ ||/\/etc\/passwd/ ||/\/etc\/gshadow/ ||/\/etc\/shadow/ ||/\/etc\/security\/opasswd/ ||/\/etc\/nsswitch.conf/ ||/\/etc\/pam.conf/ ||/\/etc\/pam.d/) &&/ +-p *wa/ &&(/ key= *[!-~]* *$/||/ -k *[!-~]* *$/) {print}' /etc/audit/rules.d/*.rules"
running_cmd="auditctl -l | awk '/^ *-w/ &&(/\/etc\/group/ ||/\/etc\/passwd/ ||/\/etc\/gshadow/ ||/\/etc\/shadow/ ||/\/etc\/security\/opasswd/ ||/\/etc\/nsswitch.conf/ ||/\/etc\/pam.conf/ ||/\/etc\/pam.d/) &&/ +-p *wa/ &&(/ key= *[!-~]* *$/||/ -k *[!-~]* *$/) {print}'"
...
if [ ${#a_output[@]} -eq 0 ] && [ ${#a_output2[@]} -eq 16 ]; then
