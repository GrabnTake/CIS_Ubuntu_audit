...
if [ -d "/etc/audit/" ]; then
    ...
    while IFS= read -r l_fname; do
        ...
        a_output2+=("- File: \"$l_fname\" is owned by group: \"$l_owner\" (should be owned by group: \"root\")")
    done < <(find /etc/audit/ -type f \( -name "*.conf" -o -name '*.rules' \) ! -group root -print)
    
    # If no violations found, report success
    if [ ${#a_output2[@]} -eq 0 ]; then
        a_output+=("- All audit configuration files in /etc/audit/ are owned by group: \"root\"")
    fi
else
