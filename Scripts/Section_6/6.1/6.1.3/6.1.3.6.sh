...
if dpkg-query -s rsyslog &>/dev/null || rpm -q rsyslog &>/dev/null; then
    ...
    # Check basic format (*.* @@<host>)
    while IFS= read -r l_log_entry; do
        a_output+=("- Basic: $l_log_entry")
    done < <(for l_logfile in "${a_config_files[@]}"; do
        grep -Hs -- "^*.*[^I][^I]*@" "$l_logfile"
    done)

    # Check advanced format (action(...target=...))
    while IFS= read -r l_log_entry; do
        a_output+=("- Advanced: $l_log_entry")
    done < <(for l_logfile in "${a_config_files[@]}"; do
        grep -PHsi -- '^\s*([^#]+\s+)?action\(([^#]+\s+)?\btarget=\"?[^#"]+\"?\b' "$l_logfile"
    done)

    if [ ${#a_output[@]} -eq 0 ]; then
        a_output2+=("- No remote log host configurations found in rsyslog configuration")
    fi
...
echo "--------------------------"
echo "Remote Log Configurations:"
...
[ "$audit_result" = "REVIEW REQUIRED" ] && echo "Review Note: Manually verify the listed remote log configurations (Basic: @@<host>, Advanced: action(target=<host>)) point to your organization's central log host."