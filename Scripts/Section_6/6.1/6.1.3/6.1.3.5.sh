...
if dpkg-query -s rsyslog &>/dev/null || rpm -q rsyslog &>/dev/null; then
    ...
    # Extract log file destinations
    while IFS= read -r l_log_entry; do
        a_output+=("- $l_log_entry")
    done < <(for l_logfile in "${a_config_files[@]}"; do
        grep -PHs -- '^\h*[^#\n\r\/:]+\/var\/log\/.*$' "$l_logfile"
    done)

    if [ ${#a_output[@]} -eq 0 ]; then
        a_output2+=("- No log file destinations found in rsyslog configuration")
    fi
...
echo "--------------------------"
echo "Configured Log Destinations:"
...
[ "$audit_result" = "REVIEW REQUIRED" ] && echo "Review Note: Manually verify the listed log destinations match local site policy and that logs are being written as expected."