...
if ! { dpkg-query -s openssh-server &>/dev/null && systemctl is-active ssh &>/dev/null; }; then
    ...
    gssapi=$(sshd -T 2>/dev/null | grep -i "permituserenvironment")

    if echo "$gssapi" | grep -qi "permituserenvironment no"; then
        audit_result="PASS"
        a_output+=("- permituserenvironment is set to no")
    else
        audit_result="FAIL"
        a_output2+=("- permituserenvironment is not set to no (found: ${gssapi:-unset})")
    fi
fi
