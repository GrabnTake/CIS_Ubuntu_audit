...
if ! { dpkg-query -s openssh-server &>/dev/null && systemctl is-active ssh &>/dev/null; }; then
    ...
    gssapi=$(sshd -T 2>/dev/null | grep -i "usepam")

    if echo "$gssapi" | grep -qi "usepam yes"; then
        audit_result="PASS"
        a_output+=("- usepam is set to yes")
    else
        audit_result="FAIL"
        a_output2+=("- usepam is not set to yes (found: ${gssapi:-unset})")
    fi
fi
